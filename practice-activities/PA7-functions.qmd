...
[Download starter .qmd template](PA7-functions.qmd)
...
**3. Write a function called `every_other()` which takes a vector and returns every other value in the vector. Include an optional argument called `start` which lets you choose where to start skipping; that is, if `start = 1`, the function returns the 1st, 3rd, 5th, etc. values and if `start = 2`, the function returns the 2nd, 4th, 6th, etc. values.**
...
**4. Write a function called `shorten()` which takes a vector, drops the values BEFORE the cumulative sum is greater than a provided number, and returns the remaining original vector values.**
